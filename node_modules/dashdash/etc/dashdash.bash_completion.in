#!/bin/bash
#
# Bash completion generated for '' at .
#
...
if type complete &>/dev/null; then
    function __completion {
        ...
        [[ -z "$__log" ]] || _log_file="$__log"
        COMPREPLY=($(COMP_CWORD="$COMP_CWORD" \
            COMP_LINE="$COMP_LINE" \
            COMP_POINT="$COMP_POINT" \
            __completer -- "${COMP_WORDS[@]}" \
            2>$_log_file)) || return $?
    }
    complete -o default -F __completion 
    ...
    compctl -K __completion 
fi


##
## This is a Bash completion file for the '' command. You can install
## with either:
##
##     cp FILE /usr/local/etc/bash_completion.d/   # Mac
##     cp FILE /etc/bash_completion.d/             # Linux
##
## or:
##
##     cp FILE > ~/..completion
